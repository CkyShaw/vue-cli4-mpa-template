...
const rootFontSize = containerWidth / (1024 / 12)
// 设置 rem 函数
function setRem() {
	...
	// 动态根字体
	let currentRootFontSize = document.documentElement.clientWidth / (containerWidth / rootFontSize)
	// 因为项目是嵌入到父级容器中的一块区域，这块区域如果不是父级容器的宽度，那么子项目还会继续缩放一次
	// 为了保持缩放一致性需要按照父子模块的宽度比例进行调整
	let finalRootFontSize = currentRootFontSize * (containerWidth / containerWidth)
